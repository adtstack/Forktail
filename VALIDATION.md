...
### GIT-203 raw/LFS/no-lazy-fetch policy and bounded cache — 2026-07-15

- 변경 파일: `src-tauri/src/git/blob.rs`, `src-tauri/src/git/repository.rs`, `src-tauri/src/domain/git.rs`, `src-tauri/src/commands/git.rs`, `src/core/gitModels.ts`, `src/core/gitModels.test.ts`, `src/core/networkPolicy.test.ts`, `specs/001-git-snapshot-integration/tasks.md`, `VALIDATION.md`.
- 수용 기준: raw blob bytes에서 canonical Git LFS v1 pointer의 version, lowercase SHA-256 OID, decimal referenced size를 엄격히 파싱해 text와 분리된 `lfsPointer` metadata 상태로 반환한다. blob runner는 기존 safe-global `--no-lazy-fetch`와 `GIT_NO_LAZY_FETCH=1` 아래 exact `cat-file -t/-s/blob`만 사용하며 `--filters`, `--textconv`, smudge나 `git-lfs` command를 구성하지 않는다. immutable document cache는 repository session 안에만 존재하고 object algorithm+full ID를 key로 쓰며 64 entries/128 MiB LRU 한도를 보수적인 string capacity와 key/entry overhead로 계산한다. close/replacement로 session이 drop되면 content도 함께 사라지고 disk/recent state에는 기록하지 않는다.
- 실패/경계 조건: 잘못된 LFS version/OID/size, extra line, CR, 1 KiB 초과 pointer 후보는 LFS로 오인하지 않고 기존 text/binary pipeline으로 보낸다. cache key는 같은 hex라도 algorithm이 다르면 분리하고 least-recent entry를 evict하며 한도를 넘는 단일 document는 저장하지 않는다. poisoned cache state는 raw detail 없이 `GIT_COMMAND_FAILED`, 누락 promisor object는 localized stderr를 파싱하지 않고 `GIT_OBJECT_MISSING_LOCAL`로 끝난다.
- 테스트 선행 증거: 구현 전 `cargo test blob`은 `GitBlobCache`와 `GitBlobContent::LfsPointer` 미정의 compile error로 실패했고 `npm run typecheck`는 `lfsPointer`가 기존 union에 없다는 TS2322로 실패했다. 구현 후 canonical pointer/near-miss/binary, algorithm-aware LRU/byte cap, 같은 session cache hit와 fresh-session miss를 검증했다. Unix temp partial-clone fixture는 executable sentinel remote를 promisor로 등록하고 object를 제거한 뒤 read가 missing-local로 끝나며 sentinel 호출이 0인지 확인했다. 별도 repository-local `.gitattributes`, filter process/smudge, textconv와 LFS sentinel도 raw read에서 모두 0회였고 두 fixture 모두 read 전후 HEAD/index/config fingerprint가 동일했다.
- 검증: `npm run check` 통과 — typecheck, frontend Vitest 55 files/363 tests, Git-tool harness 1 file/16 tests, production build. `cd src-tauri && cargo fmt --all --check` 통과, `cargo clippy --all-targets -- -D warnings` 통과, `cargo test` 133 tests 통과/0 ignored. partial-clone/helper integration은 macOS Git 2.50.1에서 수행했으며 Windows/Linux packaged/runtime 확인은 사용자 실행 항목으로 남긴다.

### GIT-301 lossless name-status changed-file service — 2026-07-15

- 변경 파일: `src-tauri/src/git/changed_files.rs`, `src-tauri/src/git/runner.rs`, `src-tauri/src/git/mod.rs`, `src-tauri/src/commands/git.rs`, `src-tauri/src/domain/git.rs`, `src-tauri/src/lib.rs`, `src/core/gitModels.ts`, `src/core/gitModels.test.ts`, `specs/001-git-snapshot-integration/tasks.md`, `VALIDATION.md`.
- 수용 기준: 두 full immutable commit ID에 exact `diff --no-ext-diff --no-textconv --name-status -z --find-renames <left> <right> --`만 실행한다. A/D/M/T는 added/deleted/modified/typeChanged, R/C는 000–100 score가 있는 renamed/copied, U는 unmerged, X와 미래 status는 unknown으로 typed 분류한다. A는 new path만, D는 old path만, M/T는 같은 raw path를 양쪽에, R/C는 서로 다른 old/new raw path를 repository session registry의 opaque identity로 등록한다. C/U/unknown은 parser forward compatibility로만 반환하고 MVP 지원 동작으로 승격하지 않았다.
- bound/DTO: runner stdout은 32 MiB, service hard limit은 1–10,000 entries다. parser는 limit을 넘은 뒤에도 bounded output 전체의 framing/status/path/duplicate를 검증하고 전체 summary count를 계산하되 응답 entries는 limit까지만 저장하며 `truncated=true`를 반환한다. typed Tauri request는 revision pair, hard limit, UI request generation을 묶고 job cancellation을 공유한다. response는 status별 count, path registry generation과 explicit nullable old/new path·similarity score를 반환한다.
- 실패/경계 조건: missing final NUL, missing rename/copy target, empty/absolute/dot-segment path, malformed/101초과 score, known status의 잘못된 suffix, duplicate old/new side path, 0/10,000초과 limit과 cap 초과를 partial success 없이 거절한다. `M<score>` rewrite score는 유효성만 확인하고 rename similarity로 오표기하지 않는다. non-zero diff는 auto-fetch 없이 `GIT_OBJECT_MISSING_LOCAL`, parser/state 오류는 raw status/path를 노출하지 않는 `GIT_COMMAND_FAILED`로 변환한다.
