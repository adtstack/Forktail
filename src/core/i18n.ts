...
export const APP_TEXT = {
  en: {
    ...
    dropReplacementStale:
      "The dropped file was not applied because this compare changed while it was opening. Review your edits, then drop the file again.",
    ...
    detachedFolderReviewCreated: "Opened the file comparison in a new window.",
    detachedFolderReviewFocused: "Brought the existing file comparison window forward.",
    ...
    navigationBackRestored: "Moved to the previous editor position.",
    navigationBackEmpty: "There is no previous editor position.",
    navigationBackAllStale: "No valid previous editor position remains.",
    navigationBackBlocked: "Finish the current dialog, then try editor Back again.",
    navigationBackDirty: "Save or discard this document before moving to another review item.",
    navigationBackInFlight: "An editor position restore is already in progress.",
    navigationBackCancelled: "Previous editor position restore was cancelled.",
    navigationBackRestoring: "Restoring the previous editor position…",
    navigationBackFailed: "Could not restore the previous editor position.",
    navigationPlatformUnavailable: "Editor Back shortcuts are unavailable in this session.",
    navigationMenuUnavailable: "The native editor Back menu is unavailable.",
    busyAria: "Working",
    errorTitle: "Error",
    doneTitle: "Done",
  ...
  ko: {
    ...
    dropReplacementStale:
      "파일을 여는 동안 비교 내용이 바뀌어 드롭한 파일을 적용하지 않았습니다. 현재 편집 내용을 확인한 뒤 다시 드롭하세요.",
    ...
    folderResults: "폴더 결과",
    detachedFolderReviewCreated: "파일 비교를 새 창에 열었습니다.",
    detachedFolderReviewFocused: "이미 열린 파일 비교 창을 앞으로 가져왔습니다.",
    recentSessionFailure: (message: string) =>
    ...
    navigationBackRestored: "이전 편집 위치로 이동했습니다.",
    navigationBackEmpty: "이동할 이전 편집 위치가 없습니다.",
    navigationBackAllStale: "유효한 이전 편집 위치가 남아 있지 않습니다.",
    navigationBackBlocked: "현재 대화상자를 마친 뒤 편집 위치 뒤로가기를 다시 시도하세요.",
    navigationBackDirty: "다른 검토 항목으로 이동하기 전에 현재 문서를 저장하거나 변경을 취소하세요.",
    navigationBackInFlight: "이미 이전 편집 위치를 복원하고 있습니다.",
    navigationBackCancelled: "이전 편집 위치 복원을 취소했습니다.",
    navigationBackRestoring: "이전 편집 위치를 복원하는 중…",
    navigationBackFailed: "이전 편집 위치를 복원하지 못했습니다.",
    navigationPlatformUnavailable: "이 세션에서는 편집 위치 뒤로가기 단축키를 사용할 수 없습니다.",
    navigationMenuUnavailable: "편집 위치 뒤로가기 메뉴를 사용할 수 없습니다.",
    busyAria: "작업 중",
    errorTitle: "오류",
    doneTitle: "완료",
...
export const CORE_TEXT = {
  en: {
    ...
    errors: {
      ...
      DETACHED_WINDOW_LIMIT:
        "Up to 8 detached comparisons can be open. Close one and try again.",
      DETACHED_SOURCE_BYTE_LIMIT:
        "Detached comparisons reached the 256 MiB source limit. Close another window and try again.",
      DETACHED_SOURCE_STALE:
        "The source folder comparison changed. Close this window and open the current row again.",
      DETACHED_UNKNOWN_WINDOW:
        "This detached comparison is no longer registered. Close it and open the row again.",
      DETACHED_WINDOW_CREATE_FAILED:
        "Could not open or focus the detached comparison window. Try again.",
      DETACHED_INVALID_STATE:
        "That action is unavailable in the current detached comparison state.",
  ...
  ko: {
    ...
    errors: {
      ...
      DETACHED_WINDOW_LIMIT:
        "별도 비교 창은 최대 8개까지 열 수 있습니다. 창을 닫은 뒤 다시 시도하세요.",
      DETACHED_SOURCE_BYTE_LIMIT:
        "별도 비교 창의 source 한도(256 MiB)에 도달했습니다. 다른 창을 닫고 다시 시도하세요.",
      DETACHED_SOURCE_STALE:
        "원본 폴더 비교가 갱신됐습니다. 이 창을 닫고 현재 행을 다시 여세요.",
      DETACHED_UNKNOWN_WINDOW:
        "이 별도 비교 창의 세션을 찾을 수 없습니다. 닫은 뒤 행을 다시 여세요.",
      DETACHED_WINDOW_CREATE_FAILED:
        "별도 비교 창을 열거나 앞으로 가져오지 못했습니다. 다시 시도하세요.",
      DETACHED_INVALID_STATE:
        "현재 별도 비교 창 상태에서는 이 작업을 실행할 수 없습니다.",
...
export const FILE_COMPARE_TEXT = {
  en: {
    ...
    folderReviewSessionAria: "Detached read-only folder review",
    folderReviewReadOnlyNote:
      "This detached folder comparison is read-only. Editing, saving, exporting, swapping, and replacing inputs are disabled.",
    swap: "Swap",
  ...
  ko: {
    ...
    folderReview: "폴더 파일 검토",
    folderReviewSessionAria: "분리된 읽기 전용 폴더 파일 검토",
    folderReviewReadOnlyNote:
      "이 별도 폴더 비교는 읽기 전용입니다. 편집, 저장, 내보내기, 좌우 교환, 입력 교체는 비활성화됩니다.",
    swap: "좌우 교환",
...
export const FOLDER_COMPARE_TEXT = {
  en: {
    ...
    completed: "Completed",
    failed: "Failed",
    phaseInventory: "Discovering folders",
    phaseClassify: "Classifying entries",
    phaseHash: "Checking file contents",
    progressDiscovered: (count: number) => `Discovered ${count.toLocaleString()}`,
    progressFinalized: (count: number) => `Final ${count.toLocaleString()}`,
    progressPending: (count: number) => `Checking ${count.toLocaleString()}`,
    progressErrors: (count: number) => `Errors ${count.toLocaleString()}`,
    progressHashed: (count: number) => `Hashed ${count.toLocaleString()}`,
    completedSummary: (count: number, durationMs: number) =>
      `${count.toLocaleString()} entries completed in ${durationMs.toLocaleString()} ms.`,
    cancelledSummary: (finalized: number, pending: number) =>
      `Partial result: ${finalized.toLocaleString()} final, ${pending.toLocaleString()} checking.`,
    job: "Job",
    cancel: "Cancel",
    ...
    interactionGuideAria: "Folder row controls",
    interactionGuideTitle: "Row controls",
    singleClick: "Single click",
    selectOnly: "Select only",
    doubleClick: "Double-click",
    activateRow: "Open file in a new window · expand or collapse folder",
    detailsKey: "Space",
    ...
    status: "Status",
    relativePath: "Folders / files (relative)",
    size: "Size",
    modified: "Modified",
    kind: "Kind",
    rowActionCompare: "Double-click or press Enter to open this file comparison in a new window",
    ...
    selected: "SELECTED",
    compare: "Open Compare Window",
    compareUnavailable: "Only final regular file rows can open in a new comparison window.",
  ...
  ko: {
    ...
    completed: "스캔 완료",
    failed: "스캔 실패",
    phaseInventory: "폴더와 파일 찾는 중",
    phaseClassify: "항목 상태 판정 중",
    phaseHash: "파일 내용 확인 중",
    progressDiscovered: (count: number) => `발견 ${count.toLocaleString()}개`,
    progressFinalized: (count: number) => `판정 완료 ${count.toLocaleString()}개`,
    progressPending: (count: number) => `확인 중 ${count.toLocaleString()}개`,
    progressErrors: (count: number) => `오류 ${count.toLocaleString()}개`,
    progressHashed: (count: number) => `해시 완료 ${count.toLocaleString()}개`,
    completedSummary: (count: number, durationMs: number) =>
      `${count.toLocaleString()}개 항목을 ${durationMs.toLocaleString()}ms에 완료했습니다.`,
    cancelledSummary: (finalized: number, pending: number) =>
      `부분 결과: 판정 완료 ${finalized.toLocaleString()}개, 확인 중 ${pending.toLocaleString()}개`,
    job: "작업",
    cancel: "스캔 취소",
    ...
    rightToLeft: "오른쪽→왼쪽",
    noActions: "작업 없음",
    interactionGuideAria: "폴더 행 조작",
    interactionGuideTitle: "행 조작",
    singleClick: "한 번 클릭",
    selectOnly: "선택만",
    doubleClick: "더블 클릭",
    activateRow: "파일은 새 창 열기 · 폴더는 펼침/접기",
    detailsKey: "Space",
    ...
    status: "상태",
    relativePath: "폴더 / 파일 (상대 경로)",
    size: "크기",
    modified: "수정 시각",
    kind: "종류",
    rowActionCompare: "더블 클릭 또는 Enter로 이 파일 비교를 새 창에 열기",
    ...
    selected: "선택됨",
    compare: "새 비교 창",
    compareUnavailable: "확정된 일반 파일 행만 새 비교 창으로 열 수 있습니다.",
    shownTotal: (shown: number, total: number) =>
