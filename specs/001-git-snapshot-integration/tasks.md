...
- [X] T043 [P] [US2] GIT-605 staged/unstaged/untracked/unmerged filter/refresh/keyboard UI 테스트를 `src/components/GitWorkingTreeFiles.test.tsx`에 먼저 추가한다
...
- [X] T047 [US2] GIT-605 working-tree changed-file UI와 three-state selector를 `src/components/GitWorkingTreeFiles.tsx`, `src/components/GitCompareView.tsx`에 구현한다
...
- [X] T050 [US3] GIT-503 stage/result fingerprint race, containment/symlink, unresolved marker, write fault, index mutation guard 테스트를 `src-tauri/src/git/conflicts.rs`와 `src-tauri/src/commands/files.rs`에 먼저 추가한다
- [X] T051 [P] [US3] GIT-604 conflict selection/save-refresh/dirty-switch/keyboard UI 테스트를 `src/components/GitConflictView.test.tsx`와 `src/components/MergeView.test.tsx`에 먼저 추가한다
...
- [X] T054 [US3] GIT-503 Result-only safe-save와 `CONFLICT_SAVED` next-step contract를 `src-tauri/src/git/conflicts.rs`, `src-tauri/src/commands/git.rs`, `src/core/mergeSave.ts`에 구현한다
- [X] T055 [US3] GIT-604 conflict list와 기존 MergeView 연결을 `src/components/GitConflictView.tsx`, `src/components/MergeView.tsx`, `src/App.tsx`에 구현한다
...
- [X] T057 [US4] GIT-702 clean/conflict/missing/binary/type-change/read-only capability 테스트를 `src-tauri/src/git/session.rs`, `src/core/gitSession.test.ts`, `src/components/MergeView.test.tsx`에 먼저 추가한다

### Implementation for User Story 4

- [X] T058 [US4] GIT-701 merge-base typed service를 `src-tauri/src/git/merge_base.rs`, `src-tauri/src/commands/git.rs`에 구현한다
- [X] T059 [US4] GIT-702 immutable Base/Left/Right preview adapter와 disclaimer UI를 `src-tauri/src/git/session.rs`, `src/core/gitSession.ts`, `src/components/MergeView.tsx`에 구현한다
...
- [X] T067 [US6] GIT-609 rename boundary/deleted path/non-UTF-8/shallow/limit/cancel/no-network file-history 테스트를 `src-tauri/src/git/history.rs`와 `src/components/GitFileHistory.test.tsx`에 먼저 추가한다
...
- [X] T069 [US6] GIT-609 bounded file-history service와 snapshot compare handoff UI를 `src-tauri/src/git/history.rs`, `src-tauri/src/commands/git.rs`, `src/components/GitFileHistory.tsx`, `src/core/gitSession.ts`에 구현한다
...
- [ ] T076 FND-006 최신 `main` CI의 frontend Linux path portability/full-suite timeout, Windows Git-tool harness parse, Rust stable clippy, Windows file-attribute compile 실패를 `src/core/privacyLoggingPolicy.test.ts`, `src/core/updaterReleaseScripts.test.ts`, `scripts/git-tool-smoke.mjs`, `rust-toolchain.toml`, `.github/workflows/ci.yml`, `src-tauri/src/git/runner.rs`, `src-tauri/src/commands/files.rs`에서 재현·수정하고 Linux/Windows required job과 `npm run check`, `cargo fmt --check`, `cargo clippy --all-targets -- -D warnings`, `cargo test`가 같은 pinned toolchain에서 모두 통과하는 증적을 남긴다 per Constitution V / plan: Test gates (contradicts)
- [ ] T077 GIT-608 `buildGitSnapshotPatch`의 WebView main-thread 동기 생성을 cancellable Web Worker job으로 분리하고 request identity로 stale 결과를 폐기하도록 `src/core/diffReport.ts`, `src/core/gitPatchJob.ts`, `src/workers/gitPatchWorker.ts`, `src/App.tsx`와 대응 테스트를 추가하며 10,000-line export 중 입력 block 100ms 미만·취소 1초 이내·repository mutation 0을 검증한다 per Constitution: Product boundary / FR-013 / SC-010 (contradicts)
- [ ] T078 GIT-402 resolved revision과 working tree의 선택 path 비교를 기존 `openGitWorkingTreeCompare` bridge에 연결하도록 `src/components/GitWorkingTreeFiles.tsx`, `src/core/gitSession.ts`, `src/App.tsx`와 component/bridge race 테스트를 보강하고 missing·symlink·external-change 실패를 명시하면서 index/worktree mutation 0을 검증한다 per FR-007 (partial)
- [ ] T079 GIT-301/GIT-603 backend가 보존한 `unknown` changed entry를 status/count와 함께 명시적 non-openable row로 유지하고 `unmerged`는 conflict 흐름, `copied`는 미지원 상태로 정직하게 분류하도록 `src/core/gitSession.ts`, `src/components/GitChangedFiles.tsx` 및 parser/component 테스트를 보강하며 opaque path identity와 filter count 일치를 검증한다 per FR-003 / FR-014 / FR-020 (contradicts)
- [ ] T080 GIT-603 binary·symlink·submodule·LFS pointer·missing local object notice에 snapshot origin, mode/object/size 또는 LFS referenced size와 상태별 행동 가능한 다음 단계를 표시하도록 `src/core/gitSession.ts`, `src/components/GitChangedFiles.tsx`, `src/components/GitWorkingTreeFiles.tsx`, `src/components/GitTreePicker.tsx` 및 접근성 테스트를 보강하고 어느 상태도 text open/export로 승격하지 않음을 검증한다 per US1 Acceptance 4 / FR-011 / FR-014 (partial)
- [ ] T081 GIT-001/GIT-601/GIT-602/GIT-603/GIT-605 repository open, ref/revision resolve, status, changed-file load를 caller-owned job ID로 실제 취소하고 child/process tree 정리와 stale-result 폐기를 일관되게 적용하도록 `src/core/bridge.ts`, `src/App.tsx`, `src-tauri/src/commands/git.rs`, `src-tauri/src/git/runner.rs` 및 cancel/timeout race 테스트를 보강하며 UI 복구와 취소 응답 1초 이내를 검증한다 per FR-013 / SR-006 / SC-002 (partial)
- [ ] T082 GIT-605/GIT-606 comparison, revision, tracked-tree left/right selection이 바뀌면 active snapshot job을 취소·무효화하고 응답 identity가 현재 repository/generation/comparison/opaque path와 모두 일치할 때만 화면을 열도록 `src/App.tsx`, `src/core/gitSession.ts`와 deferred-response component 테스트를 보강한다 per FR-013 / SC-002 (partial)
- [ ] T083 GIT-606 100,000-entry tracked tree에서 Arrow/Home/End selection과 virtual window, 실제 `scrollTop`, active descendant/focus를 동기화하도록 `src/components/GitTreePicker.tsx`, `src/components/GitTreePicker.test.tsx`를 보강하고 keyboard-only·200% 확대 fixture에서 blank viewport나 selection loss가 없음을 검증한다 per FR-015 / SC-002 / SC-008 (partial)
- [X] T084 INT-002 packaged runtime이 반환한 OS와 실제 executable path를 authoritative DTO로 사용하고 감지 성공 시 OS 선택/path 입력을 숨긴 요약+copy 흐름, 감지 실패/dev 시 manual fallback, 명시적 advanced override만 제공하도록 `src-tauri/src/commands/system.rs`, `src/core/bridge.ts`, `src/components/GitToolSetup.tsx`와 OS별 contract/component 테스트 및 `docs/02_ARCHITECTURE.md`, `docs/14_PRODUCT_GAP_ROADMAP.md`를 갱신하며 초기 오류 flash와 `.gitconfig` 자동 쓰기 0을 검증한다 per plan: Git config setup / T007 (partial)
- [ ] T085 GIT-801 동일 release artifact의 Windows/macOS/Linux clean profile에서 repository-aware revision compare/conflict save의 keyboard-only·200% 확대와 종료 후 localStorage/IndexedDB/cache/log sentinel 0건을 실행하고 artifact SHA, OS/Git version, pass/fail/manual-not-run을 `VALIDATION.md`, `docs/20_GIT_TEST_PLAN.md`, `docs/BETA_CHECKLIST.md`에 기록한다 per SC-008 / SC-009 (partial)
